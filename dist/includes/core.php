...
abstract class Core
{
	 ...
	 * @param boolean $ajax запрос является AJAX
	 * @param boolean $debug вернуть заголовки запроса и ответа
	 * @return mixed(boolean|string|array)
	 */
	public function fast_request($url, $params = false, $header = false, $method = "GET", $answer = false, $ajax = false, $debug = false)
	{
		if(! function_exists('curl_init'))
		{
			return false;
	 ...
	 *
	 * @param int $seconds время в секундах, в течение которого скрипт должен завершить работу
	 * @return boolean
	 */
