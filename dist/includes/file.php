...
class File
{
	...
	*/
	public static function file_size($file_path)
	{
		...
		if(! file_exists(ABSOLUTE_PATH.$file_path) || ! self::is_writable($file_path)) return false;
	...
	*/
	...
	{
		if(! file_exists(ABSOLUTE_PATH.$file_path)) return false;
	 ...
	 *
	 * @param string $path путь до папки относительно корня сайта
	 * @param string $f_pattern шаблон для файлов согласно правилам, используемым в функции preg_match
	 * @param boolean $coincide_f отношение файлов к шаблону: true - соответствовать шаблону, false - не соответствовать шаблону
	 * @param boolean $glob_f использовать шаблон для файлов на уровне вложенности: true - на каждой, false - только текущей
	 * @param string $d_pattern шаблон для директорий согласно правилам, используемым в функции preg_match
	 * @param boolean $coincide_f отношение директорий к шаблону: true - соответствовать шаблону, false - не соответствовать шаблону
	 * @param boolean $glob_d использовать шаблон для директорий на уровне вложенности: true - на каждой, false - только текущей
	 * @param integer $depth глубина вложенности, просматриваемая функцией (без ограничений = -1)
	 * @param integer $glob включать файлы, начинающиеся с точки, и директории-точки (точка и две точки, если есть вышестоящая директория)
	 * @return array
	 */
	public static function rglob($path = '', $f_pattern = false, $coincide_f = true, $glob_f = true, $d_pattern = false, $coincide_d = true, $glob_d = true, $depth = 0, $glob = false)
	{
		$dir_separator = '/';
		...
		foreach($files as $key => $file)
		{
			...
			if($f_pattern)
			{
				$coincide = !! preg_match($f_pattern, $file_name);
				if($coincide_f != $coincide)
				{
					unset($files[$key]);
					continue;
				}
		...
		foreach ($dirs as $key => $dir)
		{
			...
			if($d_pattern)
			{
				$coincide = !! preg_match($d_pattern, $dir_name);
				if($coincide_d != $coincide)
				{
					continue;
				}
			}
			if(! $fs = self::rglob($dir.$dir_separator, ($glob_f ? $f_pattern : false), $coincide_f, $glob_f, ($glob_d ? $d_pattern : false), $coincide_d,  $glob_d, $depth, $glob))
			{
				continue;
			}
	 ...
	 * @return mixed File size or false if error
	 */
	public static function rglob_size($path = '', $f_pattern = false, $coincide_f = true, $glob_f = true, $d_pattern = false, $coincide_d = true, $glob_d = true, $depth = 0, $glob = false)
	{
		$size = 0;
		$dir_separator = '/';
		if(defined('DIRECTORY_SEPARATOR') && DIRECTORY_SEPARATOR)
		...
		foreach ($dirs as $key => $dir)
		{
			...
			$file_size = self::rglob_size($dir.$dir_separator, ($glob_f ? $f_pattern : false), $coincide_f, $glob_f, ($glob_d ? $d_pattern : false), $coincide_d,  $glob_d, $depth, $glob);
			$diff = PHP_INT_MAX - $file_size;
			if($diff < $size)
			{
				return false;
