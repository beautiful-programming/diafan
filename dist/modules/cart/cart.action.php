...
class Cart_action extends Action
{
	...
	public function order($cart = false, $is_one_click = false)
	{
		...
		foreach ($cart["rows"] as $row)
		{
			...
			{
				DB::query("INSERT INTO {shop_order_goods_param} (order_goods_id, value, param_id) VALUES ('%d', '%d', '%d')", $shop_good_id, $value, (! empty($select_params[$value]) ? $select_params[$value] : 0));
			}
		...
		foreach ($rows_coupon as $coupon)
		{
			...
			DB::query("UPDATE {shop_order} SET coupon='%s' WHERE id=%d", $coupon["coupon"], $order_id);
		}
	 ...
	 * @return boolean
	 */
	private function check_order_amount()
	{
