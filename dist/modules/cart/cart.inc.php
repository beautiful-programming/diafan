...
class Cart_inc extends Diafan
{
        ...
        if(! isset($this->cache["cart"]))
        {
            ...
            if($cart["rows"])
            {
                ...
                if(! empty($cart["discount_total"]))
                {
                    ...
                    foreach($cart["rows"] as &$row)
                    {
                        $discount_total_price = $row["price"] * $cart["discount_total"]["discount_summ"] / $cart["old_summ_goods"];
	...
	public function get_count_good($good_id, $price_id = 0)
	{
        ...
        foreach($rows as $row)
        {
            if($row["good_id"] == $good_id && $row["price_id"] == $price_id)
            {
                $count += $row["count"];
            }
        }
	...
	/**
	 * Актуализирует цену и количество во всех сохраненных корзинах
	 * 
	 * @return boolean
	 */
    public function recalc_all($start = false, $limit = false)
    {
        $query = "SELECT * FROM {shop_cart} WHERE trash='0' ORDER BY id ASC";
        if($start !== false &&  $limit !== false)
        {
            $cart_ids = array();
            $rows = DB::query_range_fetch_all($query, $start, $limit);
            if(! $rows)
            {
                return false;
            }
            ...
            $query_goods = "SELECT * FROM {shop_cart_goods} WHERE trash='0' AND cart_id IN (".implode(',', $cart_ids).")";
        }
        else
        {
            $rows = DB::query_fetch_all($query); 
            if(! $rows)
            {
                return false;
            }
            $query_goods = "SELECT * FROM {shop_cart_goods} WHERE trash='0'";
        }
        $cart = array();
        $goods = DB::query_fetch_key_array($query_goods, "cart_id");
        foreach($goods as $cart_id => $cart_goods)
        {
            foreach($cart_goods as $good)
            {
                // $this->diafan->_shop->price_prepare_get($good_id, $params);
            }
        }
        foreach($goods as $cart_id => $cart_goods)
        {
            ...
            foreach($cart_goods as $good)
            {
                // $summ += $this->diafan->_shop->price_get($good_id, $params);
            }
            $cart[$cart_id] = array(
                "summ" => $summ,
                "count" => count($cart_goods)
            );
        }
        foreach($rows as $row)
        {
            // удаляем корзины, в которых нет товаров
            if(empty($cart[$row["id"]]))
            {
                DB::query("DELETE FROM {shop_cart} WHERE id=%d", $row["id"]);
            }
            elseif($cart[$row["id"]]["summ"] != $row["summ"] && $cart[$row["id"]]["count"] != $row["count_goods"])
            {
                DB::query("UPDATE {shop_cart} summ=%f, count_goods=%f WHERE id=%d", $cart[$row["id"]]["summ"], $cart[$row["id"]]["count"], $row["id"]);
            }
        }
        return true;
    }
