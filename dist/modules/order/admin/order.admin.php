...
class Order_admin extends Frame_admin
{
	...
	public function edit_variable_backend()
	{
		...
		if(! $this->diafan->is_new)
		{
			...
			{
				$values[$row["backend"]][$row["type"]][] = $value;
			}
		}
	...
	public function edit_variable_goods()
	{
		...
		echo '<li class="item">
			...
			{
				$coupon_discount_id = DB::query_result("SELECT discount_id FROM {shop_discount_coupon} WHERE coupon='%s' AND trash='0'", $this->diafan->values("coupon"));
				if(! $coupon_discount_id)
				{
					$coupon_discount_id = $this->diafan->values("discount_id");
				}
				echo '<br>'.$this->diafan->_('Купон').': <a href="'.BASE_PATH_HREF.'shop/discount/edit'.$coupon_discount_id.'/">'.$this->diafan->values("coupon").'</a>';
			}
			echo '</div>
...
<div class="num"><input name="discount_summ" value="'
	...
	public function save_variable_backend()
	{
		...
		foreach($rows_value as $row)
		{
			$values[$row["backend_id"]][$row["type"]][] = $value;
		}
		$this->diafan->values("backend", $values, true);
