...
echo '
<div class="main-quiz-end-details-form feedback_form">
...
if(! empty($result["rows"]))
{
    foreach ($result["rows"] as $row) //вывод полей из конструктора форм
    {
        ...
        switch ($row["type"])
        {
            ...
            case 'text':
                echo '<input class="main-quiz-end-details-form__text" type="text" placeholder="'.$row["name"].'" name="p'.$row["id"].'" value="">';
                break;
            ...
            case "phone":
                echo '<input type="tel" class="main-quiz-end-details-form__tel" placeholder="'.$row["name"].'" name="p'.$row["id"].'" value="">';
                break;
...
//Кнопка Отправить
echo '<input type="submit" value="'.$this->diafan->_('Получить консультацию', false).'" class="button solid c-btn main-quiz-end-details-form__submit">';
