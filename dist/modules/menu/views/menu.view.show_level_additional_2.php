...
// начало уровня меню
echo '<ul class="main-header-controls-nav-list-children main-header-controls-nav-list-children--level-'.$result["level"].'">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
    ...
    if ($row["active"])
    {
        ...
        echo '<li class="main-header-controls-nav-list-children-item main-header-controls-nav-list-children-item--level-'.$result["level"].' main-header-controls-nav-list-children-item--active">';
    }
    elseif ($row["active_child"])
    {
        // начало пункта меню для активного дочернего пункта
        echo '<li class="main-header-controls-nav-list-children-item main-header-controls-nav-list-children--level-'.$result["level"].' main-header-controls-nav-list-children-item--active-child">';
    }
    else
    {
        // начало любого другого пункта меню
        echo '<li class="main-header-controls-nav-list-children-item main-header-controls-nav-list-children-item--level-'.$result["level"].'">';
    }
    ...
    )
    {
        if ($row["othurl"])
        {
            echo '<a class="main-header-controls-nav-list-children-item__link" href="'.$row["othurl"].'"'.$row["attributes"].'>';
        }
        else
        {
            echo '<a class="main-header-controls-nav-list-children-item__link" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
        }
    }
            ...
            && (!$result["hide_parent_link"] || empty($result["rows"][$row["id"]]))))
    {
        ...
        echo '<span class="main-header-controls-nav-list-children-item__non-link">' . $row["name"] . '</span>';
    }
