...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	if ($row["active"])
	{
		...
		echo '<li class="top-line-nav-list-item top-line-nav-list-item--active">';
	}
	elseif ($row["active_child"])
	{
		// начало пункта меню для активного дочернего пункта
		echo '<li class="top-line-nav-list-item top-line-nav-list-item--active-child">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li class="top-line-nav-list-item top-line-nav-list-item--parent">';
	}
	else
	{
		// начало любого другого пункта меню
		echo '<li class="top-line-nav-list-item">';
	}
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="top-line-nav-list-item__link" href="'.$row["othurl"].'"'.$row["attributes"].'>';
		}
		else
		{
			echo '<a class="top-line-nav-list-item__link" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
            ...
            && (!$result["hide_parent_link"] || empty($result["rows"][$row["id"]]))))
	{
        ...
        echo '<span class="top-line-nav-list-item__non-link">' . $row["name"] . '</span>';
    }
	...
	if ($result["show_all_level"] || $row["active_child"] || $row["active"])
	{
		...
		if (empty($result['attributes']['count_level']) || $result['attributes']['count_level'] >= $menu_data["level"])
			echo $this->get('show_level_top_line_2', 'menu', $menu_data);  //ниже функция, которая выводит в цикле пункты второго уровня вложенности.
	}
}
