...
echo '<div id="mp-slider" class="main-slider__container">';
foreach ($result as $i => $row)
{
	if (! empty($row['html']) || ! empty($row['image']) || ! empty($row['swf']))
	{
		echo '<div class="main-slider-item slide'.(! $i ? ' active' : '').'" style="background-image:url('.(! empty($row['image']) ? BASE_PATH.USERFILES.'/bs/'.$row['image'] : '').')">';
			echo '<div class="main-slider-item__wrapper">';
				...
				if (! empty($row['link']))
				{
					echo '<a href="'.$row['link'].'" class="main-slider-item__btn c-btn-ghost js_bs_counter bs_counter button" rel="'.$row['id'].'" '.(! empty($row['target_blank']) ? 'target="_blank"' : '').'>'.(empty($row['title']) ? $this->diafan->_("Заказать") : $row['title']).'</a>';
				}
